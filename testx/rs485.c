...
static unsigned char  rcmd[] = {0x55, 0x55, 0xee, 0x0b, 0x03, 0x00, 0x1e, 0x10, 0x00, 0x01, 0x02, 0x00, 0x00, 0x00};

static unsigned short
u_crc16(unsigned char *buf, int len)
{
	int counter;
...
main(int argc, char *argv[])
{
	...
	len = sizeof(rcmd);
	cksum = u_crc16(&rcmd[0], len-2);
	rcmd[len-1] = cksum >> 8;
	rcmd[len-2] = cksum & 0xff;
	printf("--- crcl = 0x%02x, crch = 0x%02x ---\n", rcmd[len-2], rcmd[len-1]);

	while (1) {
		FD_ZERO(&rset);
		FD_SET(fd, &rset);
		write(fd, rcmd, sizeof(rcmd));
		tm = get_time();
